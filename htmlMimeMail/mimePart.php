...
class Mail_mimePart
{
	...
	public function __construct($body = '', $params = array())
	{
		...
		foreach ($params as $key => $value)
		{
			...
			{
				case 'content_type':
					$headers['Content-Type'] = $value . (!empty($charset) ? '; charset="' . $charset . '"' : '');
					break;
				...
				case 'dfilename':
					if (isset($headers['Content-Disposition']))
					{
						if (!empty($charset))
						{
							$headers['Content-Disposition'] .= '; filename="' . '=?' . $charset . '?B?' . base64_encode($value) . '?=' . '"';
						}
						else
						{
	 ...
	 * @access private
	 */
	protected function _quotedPrintableEncode($input, $line_max = 76)
	{
		$lines  = preg_split("/\r?\n/", $input);
